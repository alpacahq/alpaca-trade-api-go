...
func TestGetTrades_Currency(t *testing.T) {
	...
	c.do = func(c *Client, req *http.Request) (*http.Response, error) {
		resp := `{"trades":[],"currency":"HUF","symbol":"AAPL","next_page_token":null}`
		assert.Equal(t, "sip", req.URL.Query().Get("feed"))
		assert.Equal(t, "HUF", req.URL.Query().Get("currency"))
		return &http.Response{
			Body: io.NopCloser(strings.NewReader(resp)),
		}, nil
	}
...
func TestGetTrades_Update(t *testing.T) {
	c := DefaultClient
	c.do = mockResp(`{"trades":[{"t":"2022-10-21T20:19:03.752176Z","x":"D","p":129.88,"s":5,"c":[" ","T","I"],"i":71697353758036,"z":"A","u":"canceled"},{"t":"2022-10-21T20:19:03.876181Z","x":"D","p":129.88,"s":2,"c":[" ","T","I"],"i":71697353815352,"z":"A","u":"canceled"}],"symbol":"A","next_page_token":null}`)
	got, err := c.GetTrades("AAPL", GetTradesRequest{
...
func TestGetBars_Asof(t *testing.T) {
	...
	c.do = func(c *Client, req *http.Request) (*http.Response, error) {
		...
		resp := `{"bars":[{"t":"2022-06-08T04:00:00Z","o":194.67,"h":202.03,"l":194.41,"c":196.64,"v":22211813,"n":246906,"vw":198.364578},{"t":"2022-06-09T04:00:00Z","o":194.28,"h":199.45,"l":183.68,"c":184,"v":23458984,"n":281546,"vw":190.750577}],"symbol":"META","next_page_token":"TUVUQXxEfDIwMjItMDYtMDlUMDQ6MDA6MDAuMDAwMDAwMDAwWg=="}`
		switch req.URL.Query().Get("page_token") {
		case "TUVUQXxEfDIwMjItMDYtMDlUMDQ6MDA6MDAuMDAwMDAwMDAwWg==":
			resp = `{"bars":[{"t":"2022-06-10T04:00:00Z","o":183.04,"h":183.1,"l":175.02,"c":175.57,"v":27398594,"n":365035,"vw":177.335914},{"t":"2022-06-13T04:00:00Z","o":170.59,"h":172.575,"l":164.03,"c":164.26,"v":31514255,"n":436284,"vw":167.257431}],"symbol":"META","next_page_token":"TUVUQXxEfDIwMjItMDYtMTNUMDQ6MDA6MDAuMDAwMDAwMDAwWg=="}`
		case "":
		default:
...
func TestLatestBar_Feed(t *testing.T) {
	...
	c.do = func(c *Client, req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			Body: io.NopCloser(strings.NewReader(
				`{"symbol":"AAPL","bar":{"t":"2021-10-11T19:59:00Z","o":142.8,"h":142.91,"l":142.77,"c":142.8,"v":13886,"n":108,"vw":142.856726}}`,
			)),
		}, nil
	}
	_, err = c.GetLatestBar("AAPL", GetLatestBarRequest{
...
func TestLatestBar_Currency(t *testing.T) {
	...
	c.do = func(c *Client, req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			Body: io.NopCloser(strings.NewReader(
				`{"symbol":"AAPL","bar":{"t":"2023-01-14T00:59:00Z","o":2536.46,"h":2536.46,"l":2536.46,"c":2536.46,"v":441,"n":57,"vw":2536.19},"currency":"MXN"}`,
			)),
		}, nil
	}
	...
	c.do = func(c *Client, req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			Body: io.NopCloser(strings.NewReader(
				`{"symbol":"AAPL","bar":{"t":"2023-01-14T00:59:00Z","o":17289.54,"h":17289.54,"l":17289.54,"c":17289.54,"v":441,"n":57,"vw":17287.69},"currency":"JPY"}%`,
			)),
		}, nil
	}
	bar, err = c.GetLatestBar("AAPL", GetLatestBarRequest{
...
func TestLatestBars(t *testing.T) {
	...
	got, err := c.GetLatestBars([]string{"AAPL", "NIO"}, GetLatestBarsRequest{})
	require.NoError(t, err)
	require.NotNil(t, got)
	require.Len(t, got, 2)
	assert.Equal(t, Bar{
	...
	// api failure
	c.do = mockErrResp()
	got, err = c.GetLatestBars([]string{"IBM", "MSFT"}, GetLatestBarsRequest{})
	assert.Error(t, err)
	assert.Nil(t, got)
}
...
func TestLatestTrades(t *testing.T) {
	...
	got, err := c.GetLatestTrades([]string{"IBM", "MSFT"}, GetLatestTradesRequest{})
	require.NoError(t, err)
	require.NotNil(t, got)
	require.Len(t, got, 2)
	assert.Equal(t, Trade{
...
func TestLatestQuotes(t *testing.T) {
	...
	got, err := c.GetLatestQuotes([]string{"F", "GE", "TSLA"}, GetLatestQuotesRequest{})
	require.NoError(t, err)
	require.NotNil(t, got)
	require.Len(t, got, 3)
	assert.Equal(t, Quote{
...
func TestSnapshots(t *testing.T) {
	...
	got, err := c.GetSnapshots([]string{"AAPL", "MSFT", "INVALID"}, GetSnapshotsRequest{})
	require.NoError(t, err)
	require.NotNil(t, got)
	assert.Len(t, got, 3)
	...
	// api failure
	c.do = mockErrResp()
	got, err = c.GetSnapshots([]string{"AAPL", "CLDR"}, GetSnapshotsRequest{})
	assert.Error(t, err)
	assert.Nil(t, got)
}
...
func TestLatestCryptoBar(t *testing.T) {
	c := DefaultClient
	c.do = mockResp(`{"symbol":"BTC/USD","bar":{"t":"2022-02-25T12:50:00Z","o":38899.6,"h":39300,"l":38892.2,"c":39278.88,"v":74.02830613,"n":1086,"vw":39140.0960796263}}`)
...
func TestLatestCryptoBars(t *testing.T) {
	...
	got, err := c.GetLatestCryptoBars([]string{"BAT/USD", "DOGE/USD", "SUSHI/USD"}, GetLatestCryptoBarsRequest{})
	require.NoError(t, err)
	require.Len(t, got, 3)
	assert.Equal(t, CryptoBar{
...
func TestLatestCryptoTrade(t *testing.T) {
	c := DefaultClient
	c.do = mockResp(`{"symbol":"BTC/USD","trade":{"t":"2021-11-22T08:32:39.313396Z","p":57527,"s":0.0755,"tks":"B","i":17209535}}`)
...
func TestLatestCryptoTrades(t *testing.T) {
	...
	got, err := c.GetLatestCryptoTrades([]string{"BCH/USD", "ETH/USD"}, GetLatestCryptoTradesRequest{})
	require.NoError(t, err)
	require.Len(t, got, 2)
	assert.Equal(t, CryptoTrade{
		Price:     295.42,
...
func TestLatestCryptoQuote(t *testing.T) {
	c := DefaultClient
	c.do = mockResp(`{"symbol":"BCH/USD","quote":{"t":"2021-11-22T08:36:35.117453693Z","bp":564.52,"bs":44.2403,"ap":565.87,"as":44.2249}}`)
...
func TestLatestCryptoQuotes(t *testing.T) {
	...
	got, err := c.GetLatestCryptoQuotes([]string{"BTC/USD", "LTC/USD"}, GetLatestCryptoQuotesRequest{})
	require.NoError(t, err)
	require.Len(t, got, 2)
	assert.Equal(t, CryptoQuote{
...
func TestLatestCryptoSnapshots(t *testing.T) {
	...
	got, err := c.GetCryptoSnapshots([]string{"BTC/USD", "LTC/USD"}, GetCryptoSnapshotsRequest{})
	require.NoError(t, err)
	require.Len(t, got, 2)
